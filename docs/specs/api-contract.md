...
|---|---|---|---|---|---|
| POST | `/api/auth/signup` | Create auth user + subscriber + subscription atomically. No card, no Stripe. **The subscription is born `lapsed`** — ENG-999 retired the free trial, so the new account holds no access and the form sends it straight to `/checkout`. Collects **first/last name, email, phone, postcode and a password**. | `{ firstName, lastName, email, phone, postcode, password }` | `{ data: { subscriber, subscription:{ status:"lapsed", trialEndsAt } } }` | 201 · 400 `validation_failed` · 409 `account_exists` · 429 |
| POST | `/api/auth/bootstrap` | Idempotently ensure subscriber + subscription exist after a first **social** (Apple/Google/Facebook) login. Also backed by a DB trigger on `auth.users` insert. ENG-999 retired the free trial: the row is provisioned `lapsed`, and the member buys a pass at `/checkout`. | *(JWT only)* | `{ data: { subscriber, subscription } }` | 200 · 201 · 401 |
...
| POST | `/api/subscription/cancel` | Cancel at period end: Stripe `cancel_at_period_end` then the `cancel_own_subscription()` RPC. Access retained until `current_period_end`. **A row billed by the App Store / Google Play (`provider ∈ {app_store, play_store}`) answers `409 managed_by_store` before Stripe and before the RPC — nothing is written** (ENG-1192). **A complimentary row (`provider = promotional`) answers `409 complimentary` right after that check — also before Stripe and before the RPC, even with a leftover `stripe_subscription_id`; nothing is written** (ENG-1276). The RPC refusing a non-Stripe row (`42501` + `not_self_cancellable`, ENG-1221 — e.g. the provider flipped mid-request) is `409 not_self_cancellable`. | *(JWT)* `{ reason? }` | `{ data: { status:"canceled", canceledAt, currentPeriodEnd } }` | 200 · 400 `validation_failed` · 401 · 409 `no_active_subscription` · 409 `managed_by_store` · 409 `complimentary` · 409 `not_self_cancellable` · 500 `cancel_failed` · 502 `stripe_error` |
| GET | `/api/subscription/portal` | Stripe Billing Portal session pinned to `STRIPE_PORTAL_CONFIGURATION_ID` (card update only) — `302` to the portal URL. **A store-billed row answers `409 managed_by_store` (JSON, not a redirect) before any Stripe call** (ENG-1192). | *(JWT)* | `302 Location: <portal url>` | 302 · 401 · 409 `no_stripe_customer` · 409 `managed_by_store` · 502 `stripe_unavailable` / `stripe_error` |
...
`409 managed_by_store` body (both routes): `{ "error": { "code": "managed_by_store", "message": "This subscription is managed through the App Store or Google Play." } }`. The member changes their payment method or cancels in iPhone Settings → Subscriptions, or in the Google Play app. `provider` is presentation only — entitlement is still `status` + `current_period_end`.

`409 complimentary` body (cancel only): `{ "error": { "code": "complimentary", "message": "Your complimentary access ends on its own, so there's nothing to cancel." } }`. `409 not_self_cancellable` body (cancel only): `{ "error": { "code": "not_self_cancellable", "message": "This subscription can't be cancelled here." } }` — mapped only on `42501` **and** a message containing `not_self_cancellable`; any other RPC error (including a bare `42501`) stays `500 cancel_failed` with fixed copy.
